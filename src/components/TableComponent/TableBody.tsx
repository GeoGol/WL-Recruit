...
function TableBody<T extends Record<string, unknown>>({
  ...
  if (!data.length) {
    return (
      <tbody>
        <tr>
          <td colSpan={colCount} className="text-center text-md text-primary py-4">{emptyMessage ?? 'No data available.'}</td>
        </tr>
      </tbody>
    );
  }
  return (
    <tbody className="text-sm text-secondary uppercase bg-surface">
      {data.map((row, idx) => {
        ...
        return (
          <tr
            key={key}
            className={[
              '[&:not(:last-child)]:border-b [&:not(:last-child)]:border-main',
